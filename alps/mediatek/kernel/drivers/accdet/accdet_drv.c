...
void accdet_eint_work_callback(struct work_struct *work)
{
   //KE under fastly plug in and plug out
    ...
    if (cur_eint_state == EINT_PIN_PLUG_IN) {
		...
		#ifdef USE_TS3A226
		    msleep(50);
		    accdet_TS3A226_enable();  //enable GPIO119 for PIN swap 
		    ACCDET_DEBUG("[Accdet] TS3A226 enable!\n");
			msleep(200); //PIN swap need ms  
		#else
...
//EINT_PIN_PLUG_OUT
		...
		#ifdef USE_TS3A226	
		    accdet_TS3A226_disable(); //disable GPIO119 for PIN swap
		    ACCDET_DEBUG("[Accdet] TS3A226 disable!\n");
		#else
	...
	switch(accdet_status)
    {
	    ...
	    //ALPS00038030:reduce the time of remote button pressed during incoming call
			...
			case LONG_MD:
				...
				if(call_status != 0) // jeffrey modify for Bug 850
	    		{
