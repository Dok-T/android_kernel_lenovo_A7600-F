 ...
 *                : Recovery bits (0x11: wrong kernel image reset) //Image Fool Proof Mechanism
 *                : Recovery bits (0x01: wrong system image reset) //Image Fool Proof Mechanism
...
void rtc_mark_wrong_kernel_image(void)
{
       unsigned long flags;

       spin_lock_irqsave(&rtc_lock, flags);
       hal_rtc_mark_mode("wkel");
       spin_unlock_irqrestore(&rtc_lock, flags);
}

void rtc_mark_wrong_system_image(void)
{
       unsigned long flags;

       spin_lock_irqsave(&rtc_lock, flags);
       hal_rtc_mark_mode("wsys");
       spin_unlock_irqrestore(&rtc_lock, flags);
}
/*Image Fool Proof Mechanism */
