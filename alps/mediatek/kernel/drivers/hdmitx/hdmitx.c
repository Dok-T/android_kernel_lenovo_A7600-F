...
void hdmi_pll_switch(bool enable)
{
    int ret;

    HDMI_LOG("pll enable:%d\n", enable);
...
/*static*/ void hdmi_suspend(void)
{
    ...
    //disp_module_clock_off(DISP_MODULE_GAMMA, "HDMI"); // temp solution, for build error
    disp_module_clock_off(DISP_MODULE_WDMA0, "HDMI"); // temp solution, for build error
...
/*static*/ void hdmi_resume(void)
{
    ...
    if (down_interruptible(&hdmi_update_mutex)) {
		...
		DISP_LCMEnable(false);
		DPI_DisableClk();

    disp_module_clock_on(DISP_MODULE_RDMA1, "HDMI");
    //disp_module_clock_on(DISP_MODULE_GAMMA, "HDMI"); // temp solution, for build error
    disp_module_clock_on(DISP_MODULE_WDMA0, "HDMI"); // temp solution, for build error
