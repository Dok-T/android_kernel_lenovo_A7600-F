...
int cci_force_get_tbat(void)
{
    ...
    bm_print(BM_LOG_CRTI, "[cci NTC force_get_tbat] %d,%d\n", 
         bat_temperature_volt, bat_temperature_val);
    
    return bat_temperature_val;       
}

int battery_meter_get_fw_ver(void)
{
#if defined(SOC_BY_EXT_HW_FG)
    kal_int32 arg = EXT_FG_CMD_FW_VER;
...
int battery_meter_get_chem_id(void)
{
#if defined(SOC_BY_EXT_HW_FG)
    kal_int32 arg = EXT_FG_CMD_CHEM_ID;
...
int battery_meter_get_device_name(void)
{
#if defined(SOC_BY_EXT_HW_FG)
    kal_int32 arg = EXT_FG_CMD_DEVICE_NAME;
...
kal_int32 battery_meter_get_battery_current(void)
{
	...
	{
#if defined(SOC_BY_EXT_HW_FG)
	    val=EXT_FG_CMD_CURRENT;
	    ret = battery_meter_ctrl(BATTERY_METER_CMD_EXT_HW_FG, &val);
#else
...
kal_int32 cci_battery_meter_get_NTC_temperature(void)
{
    return cci_force_get_tbat();
}
...
kal_int32 battery_meter_get_battery_percentage(void)
{
    ...
    oam_run();    
        ...
        if (battery_meter_ctrl(BATTERY_METER_CMD_EXT_HW_FG, &arg) == 0)
        {
            battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] CCI_test gauge before SOC=%d \n",arg);
            if(arg<cut_percentage)
            arg=cut_percentage;
            arg=((arg-cut_percentage)*100/(100-cut_percentage));
            battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] CCI_test gauge after SOC=%d \n",arg);
            return arg;
        }
        else
            return 50;
