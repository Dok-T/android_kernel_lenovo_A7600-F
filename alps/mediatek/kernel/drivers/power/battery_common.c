...
#define CCI_BATTERY_AVERAGE_SIZE 10
#endif
...
// Bug 117 - [LVP5/LVP9] device attribute for idle station after run-in
int cci_suspend_switch = -1;
int cci_batfet_switch = -1;
// Bug 117 - [LVP5/LVP9] device attribute for idle station after run-in
// Bug [LVP5/LVP9] modify battery temperature for thermal test
int cci_batt_temp_switch = 0;
// Bug [LVP5/LVP9] modify battery temperature for thermal test
// Bug326 - For BAT ID
int cci_bat_id = -1;
// Bug326 - For BAT ID
kal_int32 cci_0_percent_bat;
kal_int32 cci_exit_low_bat;
///////////////////////////////////////////////////////////////////////////////////////////
//// Thread related 
///////////////////////////////////////////////////////////////////////////////////////////
...
extern int get_rtc_spare_fg_value();
...
//  Bug 99 - [LVP5/LVP9] device attribute for enable/disable charing function (Len Tsai)
static ssize_t show_CCIChargerSwitch(struct device *dev,struct device_attribute *attr,char *buf)
{
	return sprintf(buf,"%d\n",cci_charger_switch);
}

static ssize_t store_CCIChargerSwitch(struct device *dev, struct device_attribute *attr,const char *buf, size_t count)
{
	cci_charger_switch = simple_strtol(buf, NULL, 0);
        if(cci_charger_switch == 0)
	{
		battery_charging_control(CHARGING_CMD_SET_HIZ,&cci_charger_switch);
		battery_charging_control(CHARGING_CMD_ENABLE,&cci_charger_switch);
	}
	return count;
}
static DEVICE_ATTR(CCIChargerSwitch, 0664 , show_CCIChargerSwitch, store_CCIChargerSwitch);
//  Bug 99 - [LVP5/LVP9] device attribute for enable/disable charing function (Len Tsai)

// Bug 117 - [LVP5/LVP9] device attribute for idle station after run-in 
static ssize_t show_CCISuspendSwitch(struct device *dev,struct device_attribute *attr,char *buf)
{
	return sprintf(buf,"%d\n",cci_suspend_switch);
}

static ssize_t store_CCISuspendSwitch(struct device *dev, struct device_attribute *attr,const char *buf, size_t count)
{
	cci_suspend_switch = simple_strtol(buf, NULL, 0);
	if(cci_suspend_switch == 0)
	{
...
static DEVICE_ATTR(CCISuspendSwitch, 0664 , show_CCISuspendSwitch, store_CCISuspendSwitch);

static ssize_t show_CCIBATFETSwitch(struct device *dev,struct device_attribute *attr,char *buf)
{
	return sprintf(buf,"%d\n",cci_batfet_switch);
}

static ssize_t store_CCIBATFETSwitch(struct device *dev, struct device_attribute *attr,const char *buf, size_t count)
{
	cci_batfet_switch = simple_strtol(buf, NULL, 0);
	
	if(cci_batfet_switch == 0)
		battery_charging_control(CHARGING_CMD_SET_BATFET,&cci_batfet_switch);
	else
		battery_charging_control(CHARGING_CMD_SET_BATFET,&cci_batfet_switch);

	return count;
}
static DEVICE_ATTR(CCIBATFETSwitch, 0664 , show_CCIBATFETSwitch, store_CCIBATFETSwitch);
// Bug 117 - [LVP5/LVP9] device attribute for idle station after run-in 

// Bug [LVP5/LVP9] modify battery temperature for thermal test
static ssize_t show_CCIBATTTempSwitch(struct device *dev,struct device_attribute *attr,char *buf)
{
	return sprintf(buf,"%d\n",cci_batt_temp_switch);
}

static ssize_t store_CCIBATTTempSwitch(struct device *dev, struct device_attribute *attr,const char *buf, size_t count)
{
	cci_batt_temp_switch = simple_strtol(buf, NULL, 0);
	
	return count;
}
static DEVICE_ATTR(CCIBATTTempSwitch, 0664 , show_CCIBATTTempSwitch, store_CCIBATTTempSwitch);

static ssize_t show_CCIBATTTemp(struct device *dev,struct device_attribute *attr,char *buf)
{
	return sprintf(buf,"%d\n",BMT_status.temperature);
}

static ssize_t store_CCIBATTTemp(struct device *dev, struct device_attribute *attr,const char *buf, size_t count)
{
	if(cci_batt_temp_switch != 0)
		BMT_status.temperature = simple_strtol(buf, NULL, 0);
	
	return count;
}
static DEVICE_ATTR(CCIBATTTemp, 0664 , show_CCIBATTTemp, store_CCIBATTTemp);
// Bug [LVP5/LVP9] modify battery temperature for thermal test

// Bug326 - For BAT ID
static ssize_t show_CCIBATID(struct device *dev,struct device_attribute *attr,char *buf)
{
	if(cci_bat_id == 0)
		return sprintf(buf,"%s\n","main");
	else if(cci_bat_id == 1)
		return sprintf(buf,"%s\n","2nd");
	else if(cci_bat_id == 2)
		return sprintf(buf,"%s\n","3rd");
	else
		return sprintf(buf,"%s\n","unknown");
}

static ssize_t store_CCIBATID(struct device *dev, struct device_attribute *attr,const char *buf, size_t count)
{
	return count;
}
static DEVICE_ATTR(CCIBATID, 0664 , show_CCIBATID, store_CCIBATID);
// Bug326 - For BAT ID

static ssize_t show_FW_ver(struct device *dev,struct device_attribute *attr,char *buf)
{
    int ret_value=8888;
    ret_value = battery_meter_get_fw_ver();    
    battery_xlog_printk(BAT_LOG_CRTI, "[EM] gasgauge_firmware : %x/ \n", ret_value);
    return sprintf(buf, "%x\n", ret_value);
}
static ssize_t store_FW_ver(struct device *dev,struct device_attribute *attr,char *buf)
{
    return 0;
}

static DEVICE_ATTR(gasgauge_firmware, 0664 , show_FW_ver, store_FW_ver);


static ssize_t show_chem_id(struct device *dev,struct device_attribute *attr,char *buf)
{
    int ret_value=8888;
    ret_value = battery_meter_get_chem_id();    
    battery_xlog_printk(BAT_LOG_CRTI, "[EM] gasgauge_chem_id : %x/ \n", ret_value);
    return sprintf(buf, "%x\n", ret_value);
}
static ssize_t store_chem_id(struct device *dev,struct device_attribute *attr,char *buf)
{
    return 0;
}

static DEVICE_ATTR(gasgauge_chem_id, 0664 , show_chem_id, store_chem_id);

static ssize_t show_device_name(struct device *dev,struct device_attribute *attr,char *buf)
{
    int ret_value=8888;
    ret_value = battery_meter_get_device_name();
    battery_xlog_printk(BAT_LOG_CRTI, "[EM] gasgauge_device_name : %x/ \n", ret_value);
    if(ret_value==0x4c)
    return sprintf(buf, "celxpert\n");
    else if((ret_value==0x53))
    return sprintf(buf, "sunwoda\n");
    else
    return sprintf(buf, "unknown\n");

}
static ssize_t store_device_name(struct device *dev,struct device_attribute *attr,char *buf)
{
    return 0;
}

static DEVICE_ATTR(gasgauge_device_name, 0664 , show_device_name, store_device_name);
...
#ifdef MTK_BQ27541_SUPPORT
    if ((BMT_status.UI_SOC == 0)||((BMT_status.UI_SOC == 1)&&(g_boot_mode == NORMAL_BOOT))) {
        set_rtc_spare_fg_value(101);
    }
#else
    if ((BMT_status.UI_SOC == 0)||(BMT_status.UI_SOC == 1)) {
        set_rtc_spare_fg_value(101);
    }
...
#ifdef MTK_BQ27541_SUPPORT
static kal_uint32 cci_mt_battery_average_method(kal_uint32 *bufferdata, kal_uint32 data, kal_int32 *sum, kal_uint8 batteryIndex)
{
	kal_uint32 i;
	kal_uint32 avgdata;
	static kal_bool cci_batteryBufferFirst = KAL_TRUE;
	if (g_boot_mode == LOW_POWER_OFF_CHARGING_BOOT && BMT_status.bat_vol > 3450) 
	{ 
		g_boot_mode = KERNEL_POWER_OFF_CHARGING_BOOT;
		if(cci_exit_low_bat==0)
			cci_exit_low_bat=1;
		battery_xlog_printk(BAT_LOG_CRTI, "BAT_thread cci_exit_low_bat=%d\n",cci_exit_low_bat);
	}
	battery_xlog_printk(BAT_LOG_CRTI, "cci_exit_low_bat=%d cci_0_percent_bat=%d g_boot_mode=%d\n",cci_exit_low_bat,cci_0_percent_bat,g_boot_mode);
	if((cci_0_percent_bat==1)&&(g_boot_mode==KERNEL_POWER_OFF_CHARGING_BOOT))
	{
		*sum=0;
		data=0;
		cci_0_percent_bat=2;
		cci_batteryBufferFirst = KAL_TRUE;
	}
	else if((cci_0_percent_bat==1)&&(g_boot_mode==NORMAL_BOOT))
	{
		*sum=0;
		data=0;
		cci_0_percent_bat=2;
	...
	if(cci_batteryBufferFirst == KAL_TRUE)
	{
		for (i=0; i<CCI_BATTERY_AVERAGE_SIZE; i++)
		{
            bufferdata[i] = data;
		}

		*sum = data * CCI_BATTERY_AVERAGE_SIZE;
		cci_batteryBufferFirst = KAL_FALSE;
	}

	*sum -=	bufferdata[batteryIndex];
	*sum +=  data;
	bufferdata[batteryIndex] = data;
    avgdata = (*sum)/CCI_BATTERY_AVERAGE_SIZE;

	battery_xlog_printk(BAT_LOG_FULL, "cci bufferdata[%d]= (%d) \n", batteryIndex,bufferdata[batteryIndex]);
	return avgdata;
}
#endif
...
#ifdef MTK_BQ27541_SUPPORT
	static kal_int32 batterySOCBuffer[CCI_BATTERY_AVERAGE_SIZE];
	static kal_int32 SOC_sum;
	static kal_uint8 cci_batteryIndex = 0;
if(get_cci_hw_id()>3)
{
	BMT_status.cci_NTC_temperature = cci_battery_meter_get_NTC_temperature();
}
#endif
...
#ifdef MTK_BQ27541_SUPPORT
	battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] CCI_test gauge SOC=%d \n",SOC);
	BMT_status.SOC=cci_mt_battery_average_method(&batterySOCBuffer[0],SOC, &SOC_sum, cci_batteryIndex);
	battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] CCI_test SOC=%d BMT_status.SOC = %d\n",SOC,BMT_status.SOC);
#else
	BMT_status.SOC = SOC;	
	if (g_boot_mode == LOW_POWER_OFF_CHARGING_BOOT && BMT_status.bat_vol > 3450) 
	{ 
	...
	else if (g_boot_mode == LOW_POWER_OFF_CHARGING_BOOT && BMT_status.bat_vol <= 3450)
	{
		BMT_status.SOC=1;
		BMT_status.UI_SOC=1;
		battery_meter_reset();
...
#ifdef MTK_BQ27541_SUPPORT
	cci_batteryIndex++;
    if (cci_batteryIndex >= CCI_BATTERY_AVERAGE_SIZE)
        cci_batteryIndex = 0;
#endif
...
static void mt_battery_notify_VBatTemp_check(void)
{
...
#endif
#if defined(MTK_JEITA_STANDARD_SUPPORT)
    	if((BMT_status.temperature > MAX_CHARGE_TEMPERATURE)&&( upmu_is_chr_det() == KAL_TRUE ))
	{
	...
	if((BMT_status.temperature <= TEMP_POS_0_THRESHOLD)&&( upmu_is_chr_det() == KAL_TRUE ))
	{
	...
	if(((BMT_status.temperature > TEMP_POS_0_THRESHOLD) && ( BMT_status.temperature <= MAX_CHARGE_TEMPERATURE ))||( upmu_is_chr_det() == KAL_FALSE ))
	{
        	g_BatteryNotifyCode |= 0x0080;
	        battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] bat_temp(%d)\n", BMT_status.temperature);
	}
	else
	{
		g_BatteryNotifyCode &= ~(0x0080);
...
static void mt_battery_notify_thermal_check(void)
{
#if defined(BATTERY_NOTIFY_CASE_0006_THERMAL)
#ifdef MTK_BQ27541_SUPPORT
    if(BMT_status.cci_NTC_temperature >= 38)
    {
        g_BatteryNotifyCode |= 0x0100;
        battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] cci_NTC_temperature(%d) > 42\n", BMT_status.cci_NTC_temperature);
    }
    else
    {
        g_BatteryNotifyCode &= ~(0x0100);
    ...
    battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] BATTERY_NOTIFY_CASE_0006_THERMAL (%x)\n", g_BatteryNotifyCode);
#endif
#endif
}
...
void mt_battery_notify_check(void)
{
	...
	if(g_BN_TestMode == 0x0000)	/* for normal case */
    {
		...
		//mt_battery_notify_thermal_check();

		if(g_BatteryNotifyCode != 0x0080)
		{
			g_BatteryNotifyCode &= ~(0x0080);
		}
		if(g_BatteryNotifyCode != 0x0100)
		{
			g_BatteryNotifyCode &= ~(0x0100);
		}
    }	
...
void BAT_thread(void)
{
    ...
    if(battery_meter_initilized == KAL_FALSE)
    {
        ...
        if(get_rtc_spare_fg_value()==101)
        cci_0_percent_bat=1;
        battery_xlog_printk(BAT_LOG_CRTI, "[battery_meter_initial] cci_0_percent_bat =%d\n",cci_0_percent_bat);
    }
...
static int battery_probe(struct platform_device *dev)    
{
    ...
    /* For EM */
	{
	    ...
	    ret_device_file = device_create_file(&(dev->dev), &dev_attr_CCISuspendSwitch);
	    ret_device_file = device_create_file(&(dev->dev), &dev_attr_CCIBATFETSwitch);
	    ret_device_file = device_create_file(&(dev->dev), &dev_attr_CCIBATTTempSwitch);
	    ret_device_file = device_create_file(&(dev->dev), &dev_attr_CCIBATTTemp);
	    ret_device_file = device_create_file(&(dev->dev), &dev_attr_gasgauge_firmware);
	    ret_device_file = device_create_file(&(dev->dev), &dev_attr_gasgauge_chem_id);
	    ret_device_file = device_create_file(&(dev->dev), &dev_attr_CCIBATID);		
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_gasgauge_device_name);
	}
	...
	BMT_status.cci_NTC_temperature = 0;
