...
typedef enum
{
	charger_test_runing = 0,
	charger_test_pass,
	charger_test_fail,
	charger_test_cv
}cust_charger_test_enum;
...
static void battery_update_info(struct batteryFTM *batt, char *info)
{
    char *ptr;
	int temp = 0;
	int test_resoult=charger_test_runing;
	int charger_capacity=0;
	#ifdef FEATURE_FTM_PMIC_632X
	...
	#endif
	charger_capacity = get_int_value(BATT_CAPACITY_FILE);
	batt->bat_voltage = get_int_value(BATT_VOLT_FILE);

	/* preare text view info */
        ptr  = info;
        ptr += sprintf(ptr, "%s: %d %s \n", uistr_info_title_battery_capacity,charger_capacity,uistr_info_title_battery_percentage);	
        ...
        #endif
        if (get_int_value(AC_ONLINE_FILE))
          ptr += sprintf(ptr, "%s : %s \n", uistr_info_title_battery_chr,uistr_info_title_ac_charger);
        else if (get_int_value(USB_ONLINE_FILE))
          ptr += sprintf(ptr, "%s : %s \n", uistr_info_title_battery_chr,uistr_info_title_usb_charger);
        else
          ptr += sprintf(ptr, "%s : %s \n", uistr_info_title_battery_chr,uistr_info_title_unknow_charger);		
		...
		if(batt->is_charging)
		{
			if(batt->bat_voltage>4100)
			{
				test_resoult=charger_test_cv;		
			}
			else if(battery_fg_current<500)
			{
				test_resoult=charger_test_pass;	
			}
			else
			{
				test_resoult=charger_test_fail;
			}
		}
		switch (test_resoult) 
		{
		case charger_test_pass:
		    ptr += sprintf(ptr, "%s : %s \n", uistr_info_title_battery_test_resoult,uistr_info_title_battery_pass);
			break;
		case charger_test_fail:
		    ptr += sprintf(ptr, "%s : %s \n", uistr_info_title_battery_test_resoult,uistr_info_title_battery_fail);
			break;
		case charger_test_cv:
		    ptr += sprintf(ptr, "%s : %s \n", uistr_info_title_battery_test_resoult,uistr_info_title_battery_cv);
			break;
		case charger_test_runing:
		    ptr += sprintf(ptr, "%s : \n", uistr_info_title_battery_test_resoult);
			break;			
		}
