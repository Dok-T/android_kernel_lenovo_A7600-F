...
extern int m4u_fill_linear_pagetable(unsigned int pa, unsigned int size);
...
int disphal_allocate_fb(struct resource* res, unsigned int* pa, unsigned int* va, unsigned int* dma_pa)
{
    ...
    *va = (unsigned int) ioremap_nocache(res->start, res->end - res->start + 1);
    if (disp_use_mmu)
    {
        // alloc mva=pa to avoid flicker when lk switch to kernel, so ovl do not need to stop
    	if (LCM_TYPE_DPI == lcm_params->type)
        {   
            m4u_fill_linear_pagetable(*pa, (res->end - res->start + 1));
		    *dma_pa = *pa;
        }
		else
        {      
