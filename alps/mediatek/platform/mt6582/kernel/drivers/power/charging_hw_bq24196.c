...
kal_uint16 cv_register_value = 0x32;
#else
kal_uint16 cv_register_value = 0x2C;
#endif
...
static void hw_bc11_dump_register(void)
{
 ...
 static kal_uint32 charging_hw_init(void *data)
 {
	...
	// Modify the precharge current from 512mA to 256mA
	bq24196_set_iprechg(0x1); //Precharge current 256mA
	bq24196_set_iterm(0x0); //Termination current 128mA

#if defined(HIGH_BATTERY_VOLTAGE_SUPPORT)
    bq24196_set_vreg(cv_register_value); //VREG 4.304V
#else
    bq24196_set_vreg(cv_register_value); //VREG 4.208V
#endif    
 ...
 */
	...
	cv_register_value = charging_parameter_to_value(VBAT_CV_VTH, GETARRAYNUM(VBAT_CV_VTH), cv_value);
	bq24196_set_vreg(cv_register_value); 
...
static kal_uint32 charging_set_hiz(void *data)
{
	kal_uint32 status = STATUS_OK;
	kal_uint32 enable = *(kal_uint32*)(data);

	if(enable == KAL_TRUE)
		bq24196_set_en_hiz(0x0); // enable charging
	else
		bq24196_set_en_hiz(0x1); // disable charging
...
static kal_uint32 charging_set_batfet(void *data)
{
	kal_uint32 status = STATUS_OK;
	kal_uint32 enable = *(kal_uint32*)(data);

	if(enable == KAL_TRUE)
		bq24196_set_batfet_disable(0x0); // enable BATFET Q4
	else
		bq24196_set_batfet_disable(0x1); // disable BATFET Q4
