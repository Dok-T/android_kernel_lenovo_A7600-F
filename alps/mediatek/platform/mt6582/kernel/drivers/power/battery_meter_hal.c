...
static kal_int32 ext_fg_get_current(void *data)
{
    ...
    if(ret == 1)    
    {
        if(returnData > 32767)
        {
        returnData=((65536-returnData)*(-1));
        }
        bm_print(BM_LOG_CRTI, "[ext_fg_get_current] bq27541 read raw data = %d\n", returnData);
        *(kal_int32 *)(data) = (returnData);
        return STATUS_OK;
    }
    else
...
#ifdef EXT_FG_HAS_FW_VER
static kal_int32 ext_fg_get_fw_ver(void *data)
{
#ifdef MTK_BQ27541_SUPPORT
    int ret = 0;
    int returnData = 0;
    
    ret = bq27541_set_cmd_write(BQ27541_CMD_Control,BQ27541_SUBCMD_FW_VER);		
    ret = bq27541_set_cmd_read(BQ27541_CMD_Control,&returnData);		
    bm_print(BM_LOG_CRTI,"[BQ27541]BQ27541_SUBCMD_FW_VER = %X\n", returnData);
    if(ret == 1)    
	{
        bm_print(BM_LOG_CRTI, "[ext_fg_get_fw_ver] bq27541 read raw data = %x\n", returnData);
        *(kal_int32 *)(data) = returnData;
        return STATUS_OK;
	}
	else
	{
	    bm_print(BM_LOG_CRTI, "[ext_fg_get_fw_ver] bq27541 read raw data STATUS_UNSUPPORTED\n");
	    return STATUS_UNSUPPORTED;
    }        
#endif
    return STATUS_UNSUPPORTED;
...
#ifdef EXT_FG_HAS_CHEM_ID
static kal_int32 ext_fg_get_chem_id(void *data)
{
#ifdef MTK_BQ27541_SUPPORT
    int ret = 0;
    int returnData = 0;
    
    ret = bq27541_set_cmd_write(BQ27541_CMD_Control,BQ27541_SUBCMD_CHEM_ID);		
    ret = bq27541_set_cmd_read(BQ27541_CMD_Control,&returnData);		
    bm_print(BM_LOG_CRTI,"[BQ27541]BQ27541_SUBCMD_CHEM_ID = %X\n", returnData);
    if(ret == 1)    
	{
        bm_print(BM_LOG_CRTI, "[ext_fg_get_chem_id] bq27541 read raw data = %x\n", returnData);
        *(kal_int32 *)(data) = returnData;
        return STATUS_OK;
	}
	else
	{
	    bm_print(BM_LOG_CRTI, "[ext_fg_get_chem_id] bq27541 read raw data STATUS_UNSUPPORTED\n");
	    return STATUS_UNSUPPORTED;
    }
#endif
    return STATUS_UNSUPPORTED;
...
#ifdef EXT_FG_HAS_DEVICE_NAME
static kal_int32 ext_fg_get_device_name(void *data)
{
#ifdef MTK_BQ27541_SUPPORT
    int ret = 0;
    int returnData = 0;
    ret = bq27541_set_cmd_read(BQ27541_EXTCMD_DEVICE_NAME,&returnData);
    returnData=(returnData&0xFF);		
    bm_print(BM_LOG_CRTI,"[BQ27541]BQ27541_EXTCMD_DEVICE_NAME = %X\n", returnData);
    if(ret == 1)    
	{
        bm_print(BM_LOG_CRTI, "[ext_fg_get_device_name] bq27541 read raw data = %x\n", returnData);
        *(kal_int32 *)(data) = returnData;
        return STATUS_OK;
	}
	else
	{
	    bm_print(BM_LOG_CRTI, "[ext_fg_get_device_name] bq27541 read raw data STATUS_UNSUPPORTED\n");
	    return STATUS_UNSUPPORTED;
    }        
#endif
    return STATUS_UNSUPPORTED;
...
#endif
#ifdef EXT_FG_HAS_FW_VER
        case EXT_FG_CMD_FW_VER:
            return ext_fg_get_fw_ver(data);
            break;
#endif
#ifdef EXT_FG_HAS_CHEM_ID
        case EXT_FG_CMD_CHEM_ID:
            return ext_fg_get_chem_id(data);
            break;
#endif
#ifdef EXT_FG_HAS_DEVICE_NAME
        case EXT_FG_CMD_DEVICE_NAME:
            return ext_fg_get_device_name(data);
            break;
#endif
