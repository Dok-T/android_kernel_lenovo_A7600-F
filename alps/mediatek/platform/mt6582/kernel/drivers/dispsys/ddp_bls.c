...
// LCM ID for display backlight
static enum DISP_BLS_LCM_ID {
    DISP_BLS_LVP5_LCM_ID_MAIN,
    DISP_BLS_LVP5_LCM_ID_SECOND,
    DISP_BLS_LVP9_LCM_ID_MAIN,
    DISP_BLS_LVP9_LCM_ID_SECOND,
    DISP_BLS_LCM_ID_INVALID = 0xFFFFFFFF,
} disp_bls_lcm_id = DISP_BLS_LCM_ID_INVALID; // Default: Main source LCM
...
static unsigned int brightness_mapping(unsigned int level)
{
    ...
    if(mapped_level >= 50) {
        if (disp_bls_lcm_id == DISP_BLS_LVP9_LCM_ID_MAIN) {
            mapped_level = mapped_level + 20;
        } else if (disp_bls_lcm_id == DISP_BLS_LVP9_LCM_ID_SECOND) {
            mapped_level = mapped_level + 16;
        }
    }
...
// LCM_ID for display backlight
static int __init disp_bls_get_lcm_id(char *str)
{
    if (!strnicmp(str, "1-hx8896a03_dsi_vdo", 19)) {
        disp_bls_lcm_id = DISP_BLS_LVP9_LCM_ID_MAIN;
    } else if (!strnicmp(str, "1-nt71397_dsi_vdo", 17)) {
        disp_bls_lcm_id = DISP_BLS_LVP9_LCM_ID_SECOND;
    } else if (!strnicmp(str, "1-auo_s6d7aa0x04_dsi_vdo", 24)) {
        disp_bls_lcm_id = DISP_BLS_LVP5_LCM_ID_MAIN;
    } else if (!strnicmp(str, "1-cpt_s6d7aa0x04_dsi_vdo", 24)) {
        disp_bls_lcm_id = DISP_BLS_LVP5_LCM_ID_SECOND;
    }else {
        disp_bls_lcm_id = DISP_BLS_LCM_ID_INVALID;
        DISP_ERR("%s: Unknown Backlight LCM ID!\n", __func__);
    }
    DISP_MSG("%s: Backlight LCM ID: [%d] (lcm=%s)\n", __func__, disp_bls_lcm_id, str);

    return 1;
}

__setup("lcm=", disp_bls_get_lcm_id);
