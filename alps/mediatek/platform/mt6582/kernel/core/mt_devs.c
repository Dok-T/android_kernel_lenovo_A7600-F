...
/* BUG10 - Implement API for HW_ID Start*/
#include <linux/proc_fs.h>
/* BUG10 - Implement API for HW_ID End*/
/*Image Fool-Proof Mechanism */
...
int cci_sku_id_read(char *page, char **start, off_t off, int count, int *eof, void *data);
int cci_rf_id_read(char *page, char **start, off_t off, int count, int *eof, void *data);

// Bug326 - For BAT ID
extern int cci_bat_id;
// Bug326 - For BAT ID
...
int get_cci_sku_id(void)
{
        return cci_sku_id;
}
EXPORT_SYMBOL(get_cci_sku_id);
...
static int __init cci_sku_id_from_cmdline(char *cci_skuid)
{
        sscanf(cci_skuid, "%d", &cci_sku_id);
        printk(KERN_ERR "Read CCI SKU ID =%d from cmdline!!\n",cci_sku_id);

        return 1;
}
__setup("CCI_SKU_ID=", cci_sku_id_from_cmdline);
...
// Bug326 - For BAT ID
static int __init cci_bat_id_from_cmdline(char *cci_batid)
{
	sscanf(cci_batid, "%d", &cci_bat_id);
        printk(KERN_ERR "Read CCI BAT ID =%d from cmdline!!\n",cci_bat_id);

        return 1;
}
__setup("CCI_BAT_ID=", cci_bat_id_from_cmdline);
// Bug326 - For BAT ID
...
int cci_sku_id_read(char *page, char **start, off_t off, int count, int *eof, void *data)
{
        ...
        switch(cci_sku_id){
        case SKU_01:
                len += sprintf(page, "SKU_01\n");
                break;

        case SKU_1_1:
                len += sprintf(page, "SKU_1_1\n");
                break;

        case SKU_02:
                len += sprintf(page, "SKU_02\n");
                break;

        case SKU_03:
                len += sprintf(page, "SKU_03\n");
                break;

        case SKU_04:
                len += sprintf(page, "SKU_04\n");
                break;

        case SKU_05:
                len += sprintf(page, "SKU_05\n");
                break;

        case SKU_06:
                len += sprintf(page, "SKU_06\n");
                break;

        case SKU_07:
                len += sprintf(page, "SKU_07\n");
                break;

        case SKU_08:
                len += sprintf(page, "SKU_08\n");
                break;

        case SKU_09:
                len += sprintf(page, "SKU_09\n");
                break;

        case SKU_10:
                len += sprintf(page, "SKU_10\n");
                break;

        case SKU_11:
                len += sprintf(page, "SKU_11\n");
                break;

        case SKU_12:
                len += sprintf(page, "SKU_12\n");
                break;

        case SKU_13:
                len += sprintf(page, "SKU_13\n");
                break;

        case SKU_14:
                len += sprintf(page, "SKU_14\n");
                break;

        case SKU_15:
                len += sprintf(page, "SKU_15\n");
                break;

        case SKU_16:
                len += sprintf(page, "SKU_16\n");
                break;

        default:
                len += sprintf(page, "SKU_ID_INVALID\n");
                printk(KERN_EMERG "Got an invalid SKU id %d\n", cci_sku_id);
                break;
        }
        return len;
}
...
int cci_rf_id_read(char *page, char **start, off_t off, int count, int *eof, void *data)
{
        ...
        switch(cci_rf_id){
        case LVP9_900:
                len += sprintf(page, "LVP9_900\n");
                break;
        case LVP9_850:
                len += sprintf(page, "LVP9_850\n");
                break;
        case LVP5:
                len += sprintf(page, "LVP5\n");
                break;
        case WIFI:
                len += sprintf(page, "WIFI\n");
                break;
        default:
...
#ifdef _LVP9_
struct platform_device sensor_hallsensor = {
	.name	       = "em6781",
	.id            = -1,
};
#endif

//#if defined (_LVP5_PRC_3GFULL_) || defined (_LVP5_ROW_3GFULL_) || defined (_3GDATA_SMS_) || defined (_LVP9_PRC_3GFULL_)
#if defined (_LVP5_PRC_3GFULL_)
struct platform_device sensor_capsensor = {
	.name	       = "iqs128",
	.id            = -1,
};
#endif
#if defined (_LVP5_ROW_3GFULL_)
struct platform_device sensor_capsensor = {
	.name	       = "iqs128",
	.id            = -1,
};
#endif
#if defined (_LVP9_ROW_3GDATASMS_)
struct platform_device sensor_capsensor = {
	.name	       = "iqs128",
	.id            = -1,
};
#endif
#if defined (_LVP5_ROW_3GDATASMS_)
struct platform_device sensor_capsensor = {
	.name	       = "iqs128",
	.id            = -1,
};
#endif
#if defined (_LVP9_PRC_3GFULL_)
 struct platform_device sensor_capsensor = {
 	.name	       = "iqs128",
 	.id            = -1,
 };
...
/* BUG10 - Implement API for HW_ID Start */
    ...
    create_proc_read_entry("CCI_SKU_ID", 0, NULL, cci_sku_id_read, NULL);
...
/*Image Fool-Proof Mechanism */
#if defined (_LVP5_ROW_3GDATASMS_)||defined (_LVP5_ROW_3GFULL_)||defined (_LVP5_PRC_3GFULL_)
    if (((SKU_01<= get_cci_sku_id())&&(SKU_06>=get_cci_sku_id())) || ((SKU_09<= get_cci_sku_id())&&(SKU_11>=get_cci_sku_id())))
    {
        printk("===== Kernel Image Correct!! LVP5 3G===== \n");
    }
    else
    {
        printk("===== Kernel Image Incorrect!! Need LVP5 3G Kernel Image===== \n");
        arch_reset(0,"wrong_kernel_image");
               while(1);
    }

#elif defined(_LVP5_ROW_WIFIONLY_)
    if ( SKU_07== get_cci_sku_id() || SKU_08== get_cci_sku_id())
    {
        printk("===== Kernel Image Correct!! LVP5 WIFI ===== \n");
    }
    else
    {
        printk("===== Kernel Image Incorrect!! Need LVP5 WIFI Kernel Image===== \n");
        arch_reset(0,"wrong_kernel_image");
               while(1);
    }

#elif defined (_LVP9_ROW_3GDATASMS_) || defined (_LVP9_PRC_3GFULL_)
    if ((SKU_12<= get_cci_sku_id())&&(SKU_14>=get_cci_sku_id()))
    {
        printk("===== Kernel Image Correct!! LVP9 3G===== \n");
    }
    else
    {
        printk("===== Kernel Image Incorrect!! Need LVP9 3GKernel Image===== \n");
        arch_reset(0,"wrong_kernel_image");
               while(1);
    }
#elif defined(_LVP9_PRC_WIFIONLY_) || defined(_LVP9_ROW_WIFIONLY_)
    if ( SKU_15== get_cci_sku_id() || SKU_16== get_cci_sku_id())
    {
        printk("===== Kernel Image Correct!! LVP9 WIFI ===== \n");
    }
    else
    {
        printk("===== Kernel Image Incorrect!! Need LVP9 WIFI Kernel Image===== \n");
        arch_reset(0,"wrong_kernel_image");
               while(1);
    }
#else
        printk("===== PROJ_ID in PROJECTCONFIG NOT DEFINED ===== \n");
        arch_reset(0,"wrong_kernel_image");
        while(1);
#endif
/*Image Fool-Proof Mechanism */
...
#endif
#ifdef _LVP9_
	retval = platform_device_register(&sensor_hallsensor);
		printk("sensor_hallsensor device!");
	if (retval != 0)
		return retval;
#endif
//#if defined (_LVP5_PRC_3GFULL_) || defined (_LVP5_ROW_3GFULL_) || defined (_3GDATA_SMS_) || defined (_LVP9_PRC_3GFULL_)
#if defined (_LVP5_PRC_3GFULL_)
	retval = platform_device_register(&sensor_capsensor);
		printk("sensor_capsensor device!");
	if (retval != 0)
		return retval;
#endif
#if defined (_LVP5_ROW_3GFULL_)
	...
	retval = platform_device_register(&hid_attr);
		printk("hid_attr device!");
	if (retval != 0)
		return retval;
